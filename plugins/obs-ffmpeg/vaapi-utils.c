...
static VADisplay vaapi_open_display_drm(const char *device_path)
{
	VADisplay va_dpy;
...
static void vaapi_close_display_drm()
{
	if (drm_fd < 0)
		return;
...
VADisplay vaapi_open_device(const char *device_path, const char *func_name)
{
	...
	va_dpy = vaapi_open_display_drm(device_path);
	if (!va_dpy)
		return NULL;
...
bool vaapi_device_h264_supported(const char *device_path)
{
	...
	va_dpy = vaapi_open_device(device_path, "vaapi_device_h264_supported");
	if (!va_dpy)
		return false;

	ret = vaapi_display_h264_supported(va_dpy, device_path);

	vaapi_close_device(va_dpy);
