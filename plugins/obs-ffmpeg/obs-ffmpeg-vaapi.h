...
typedef bool (*valid_format_t)(enum video_format format);

static void vaapi_video_info(void *data, struct video_scale_info *info,
			     valid_format_t valid_format,
			     enum video_format default_format)
{
...
static bool
vaapi_update(void *data, obs_data_t *settings,
	     bool (*init_codec)(struct vaapi_encoder *enc, const char *path),
	     valid_format_t valid_format, enum video_format default_format)
{
	struct vaapi_encoder *enc = data;
	...
	vaapi_video_info(enc, &info, valid_format, default_format);
...
//typedef bool (*init_codec_t) (struct vaapi_encoder *enc, const char *path);
typedef bool (*update_t)(void *data, obs_data_t *settings);

static void *vaapi_create(obs_data_t *settings, obs_encoder_t *encoder,
			  const char *name, update_t update)
{
...
typedef void (*extract_headers_t)(const uint8_t *packet, size_t size,
				  uint8_t **new_packet_data,
				  size_t *new_packet_size,
				  uint8_t **header_data, size_t *header_size,
				  uint8_t **sei_data, size_t *sei_size);
typedef bool (*is_keyframe_t)(const uint8_t *data, size_t size);

static bool vaapi_encode(void *data, struct encoder_frame *frame,
			 struct encoder_packet *packet, bool *received_packet,
			 extract_headers_t extract_headers,
			 is_keyframe_t is_keyframe)
{
	struct vaapi_encoder *enc = data;
	...
	if (got_packet && enc->packet->size) {
		if (enc->first_packet) {
			...
			extract_headers(enc->packet->data, enc->packet->size,
		...
		packet->keyframe = is_keyframe(packet->data, packet->size);
...
typedef void (*add_profile_property_t)(obs_property_t *list);

static obs_properties_t *
vaapi_properties(void *unused, add_profile_property_t add_profile_property)
{
